...
enum Refl_t { DIFF,
              SPEC,
              REFR }; // material types, used in radiance(num_spheres, )
struct Sphere {
    float rad;   // radius
    Vec p, e, c; // position, emission, color
    Refl_t refl; // reflection type (DIFFuse, SPECular, REFRactive)
    Sphere() {}
    __host__ __device__ Sphere(float rad_, Vec p_, Vec e_, Vec c_, Refl_t refl_) : rad(rad_), p(p_), e(e_), c(c_), refl(refl_) {}
...
__host__ __device__ inline float clamp(float x) { return x < 0 ? 0 : x > 1 ? 1
                                                                           : x; }
