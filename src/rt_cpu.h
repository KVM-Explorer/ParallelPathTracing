        ...
        for (int y = 0; y < image.height; y++) {                                  // Loop over image rows
            ...
            for (int x = 0; x < image.width; x++) // Loop cols
            {
                if (enableAA) {
                    ...
                    Vec d = camera.cx * (x / image.width - 0.5) +
                            camera.cy * (y / image.height - 0.5) + camera.position;
                    auto ret = tracing(Ray(camera.position + d * 140, d.norm()), 0, scene);
                    image.write(x, image.height - y - 1, Vec(clamp(ret.x), clamp(ret.y), clamp(ret.z)));
                }
            }
        }
        std::cout << "\n";
